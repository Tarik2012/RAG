...
SKIP_DIRS = {
    ...
    "__pycache__", ".next", "target", "vendor", ".pytest_cache", "evals",
}
...
SKIP_FILENAMES = {
    "package-lock.json", "yarn.lock", "poetry.lock", "Pipfile.lock", ".DS_Store", "__init__.py",
}
...
def list_repo_code_files(owner: str, repo: str, branch: str | None = None) -> list[str]:
    ...
    for item in tree:
        ...
        if item.get("size", 0) > MAX_FILE_BYTES:
            continue
        if item.get("size", 0) < MIN_FILE_BYTES:
            continue
        files.append(path)
...
def fetch_file_content(owner: str, repo: str, path: str, branch: str | None = None) -> str:
